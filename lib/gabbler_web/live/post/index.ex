defmodule GabblerWeb.Live.Post.Index do
  ...
  defp init(
         %{room: room, user: user, post: post, mode: mode, op: op, focus_hash: focus_hash},
         socket
       ) do
  ...
  defp init(%{room: _, post: post, mode: _} = session, socket),
    do: init(Map.put(session, :op, post), socket)
