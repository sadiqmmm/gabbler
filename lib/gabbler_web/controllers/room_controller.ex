defmodule GabblerWeb.RoomController do
  ...
  def room(%{assigns: %{user: user}} = conn, %{"room" => name, "mode" => "live"}) do
    case QueryRoom.get(name) do
      nil -> room_404(conn)
      room -> live_render(conn, GabblerWeb.Live.Room.Index, session: %{room: room, user: user, mode: :live})
    end
  end
  ...
  def room(%{assigns: %{user: user}} = conn, %{"room" => name, "mode" => "new"}) do
    case QueryRoom.get(name) do
      nil -> room_404(conn)
      room -> live_render(conn, GabblerWeb.Live.Room.Index, session: %{room: room, user: user, mode: :new})
    end
  end
