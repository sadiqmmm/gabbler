defmodule GabblerWeb.PostController do
  ...
  defp post_render(%{assigns: %{user: user}} = conn, %{:room_name => name, :hash => hash, :mode => mode}) do
    case {QueryRoom.get(name), QueryPost.get(hash)} do
      {room, post} when is_nil(room) or is_nil(post) ->
        post_404(conn)
      {room, post} ->
        live_render(conn, GabblerWeb.Live.Post.Index, session: %{user: user, room: room, post: post, mode: mode, focus_hash: nil})
    end
  end

  defp post_render(%{assigns: %{user: user}} = conn, %{:room_name => name, :hash => hash, :focus_hash => focus_hash}) do
    case {QueryRoom.get(name), QueryPost.get(hash), QueryPost.get(focus_hash)} do
      {room, op, post} when is_nil(room) or is_nil(op) or is_nil(post) ->
        post_404(conn)
      {room, op, post} ->
        live_render(conn, GabblerWeb.Live.Post.Index, session: %{user: user, room: room, op: op, post: post, mode: :new, focus_hash: focus_hash})
    end
  end
